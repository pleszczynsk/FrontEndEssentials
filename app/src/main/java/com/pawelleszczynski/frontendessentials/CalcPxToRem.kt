...
class CalcPxToRem : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?)
    {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.calc_px_to_rem)

        if(savedInstanceState != null) {

            output_px_to_rem?.text = savedInstanceState?.get("saved1") as CharSequence?
    ...
    {
        output_rem_to_px.text = ( (BigDecimal(input_text_view2.text.toString().toDouble()/(input_text_view1.text.toString().toDouble())).setScale(2, RoundingMode.HALF_EVEN)).toString())
    }

    override fun onSaveInstanceState(@NonNull outState: Bundle) {
        super.onSaveInstanceState(outState)
        outState.putString("saved1", output_px_to_rem.text.toString())
