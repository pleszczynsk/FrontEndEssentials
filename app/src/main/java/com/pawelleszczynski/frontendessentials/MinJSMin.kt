...
class MinJSMin: AppCompatActivity() {
    private val job = Job()
    private val scope = CoroutineScope(Dispatchers.Main + job)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.minunmin_js_min)
    ...
    suspend fun jsMinify(inputstr: Editable)= withContext(Dispatchers.IO){
        ...
        val request =
            (URL("https://javascript-minifier.com/raw").openConnection() as HttpURLConnection).apply {
                requestMethod = "POST"
                doOutput = true
    ...
    fun onSendJSMinify(view: View) {
        scope.launch { output_minify.text = jsMinify(input_text_view1.text) }
    }
}