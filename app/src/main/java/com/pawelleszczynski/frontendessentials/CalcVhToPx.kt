...
import kotlinx.android.synthetic.main.calc_vh_to_px.*
import java.math.BigDecimal
import java.math.RoundingMode

class CalcVhToPx : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?)
    {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.calc_vh_to_px)

        if(savedInstanceState != null) {

            output_vh_to_px?.text = savedInstanceState?.get("saved1") as CharSequence?
    ...
    {
        output_vh_to_px.text = (( BigDecimal((input_text_view2.text.toString().toDouble() * input_text_view1.text.toString().toDouble())/100).setScale(2,
            RoundingMode.HALF_EVEN) ).toString())
    }

    override fun onSaveInstanceState(@NonNull outState: Bundle) {
        super.onSaveInstanceState(outState)
        outState.putString("saved1", output_vh_to_px.text.toString())
